...
@file dlist.h
@brief 
Definitions of a generic double linked-list ADT
...
_dlist_destroy_ is called. For example, if the list contains data dynamically
...
The _dlist_destroy_ operation removes all elements from a doubly linked-list and
calls the function passed as _destroy_ to _dlist_init_ once for each element as 
it is removed, provided _destroy_ was not set to NULL.

@note
No operation is permitted after _dlist_destroy()_ is called unless _dlist_init()_
is called again. 
...
/**
MACRO that evaluates to the prev element given an element in a linked-list
*/
